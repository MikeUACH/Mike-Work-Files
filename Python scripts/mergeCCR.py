    ...
    for archivo in archivos_acum:
        ...
        if match:
            ...
            if key in dataframes_xls:
                ...
                # Realizar un merge left para combinar con el archivo acumulado
                dataframes_xls[key] = pd.merge(dataframes_xls[key], df_acum_filtered, on='TimeStarted', how='left', suffixes=('', '_new'))

                # Actualizar solo las columnas relevantes y eliminar las temporales
                for col in df_acum_filtered.columns:
                    if col != 'TimeStarted':  # No actualizar la columna de fecha
                        # Actualizar solo las columnas de los nuevos datos
                        dataframes_xls[key][col] = dataframes_xls[key][f'{col}_new'].combine_first(dataframes_xls[key][col])
                
                # Eliminar las columnas temporales generadas por el merge
                dataframes_xls[key].drop(columns=[f'{col}_new' for col in df_acum_filtered.columns if col != 'TimeStarted'], inplace=True)
